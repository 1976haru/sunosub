 ...
 *
 * TASK CS-v1.8 — also tracks byTier (free vs paid call counts) and
 * tokens.paid (input/output token totals, paid tier only — see
 * recordGeminiUsage()'s doc comment). Still display-only *from this file's
 * point of view*: it only records. lib/geminiLimits.js + lib/gemini.js's
 * requireGeminiClient() are what actually read byTier.paid back out to
 * enforce the user-set daily cap — this file has no opinion on whether a
...
const emptyDay = () => ({ total: 0, failed: 0, byLabel: {}, byTier: { free: 0, paid: 0 }, tokens: { paid: { input: 0, output: 0 } } });

// TASK CS-v1.8 — days already on disk from before byTier/tokens existed
// (or a manually-edited file missing them) shouldn't crash a `day.byTier.x += 1`
// with "cannot set property of undefined" — backfill the shape on read.
function normalizeDay(day) {
  return {
    total: Number(day?.total) || 0,
    failed: Number(day?.failed) || 0,
    byLabel: day?.byLabel && typeof day.byLabel === 'object' ? day.byLabel : {},
    byTier: { free: Number(day?.byTier?.free) || 0, paid: Number(day?.byTier?.paid) || 0 },
    tokens: { paid: { input: Number(day?.tokens?.paid?.input) || 0, output: Number(day?.tokens?.paid?.output) || 0 } },
  };
}
 ...
 *
 * TASK CS-v1.8 — `tier` now also buckets the attempt into byTier, and for a
 * successful PAID-tier call, `usageMetadata` (the raw object Gemini's
 * response carries — promptTokenCount/candidatesTokenCount/
 * thoughtsTokenCount) is folded into tokens.paid. Free-tier and failed
 * calls never touch tokens.paid — there's nothing billed to record.
 * Thinking tokens are counted as output because Gemini bills them as
 * output tokens, not separately.
 */
export function recordGeminiUsage({ label = '', success = true, tier = 'free', usageMetadata = null } = {}, now = new Date()) {
  const usage = loadUsage();
  const key = todayKey(now);
  const day = normalizeDay(usage.days[key]);
  day.total += 1;
  ...
  day.byTier[tierKey] += 1;
  if (tierKey === 'paid' && usageMetadata) {
    day.tokens.paid.input += Number(usageMetadata.promptTokenCount) || 0;
    day.tokens.paid.output += (Number(usageMetadata.candidatesTokenCount) || 0) + (Number(usageMetadata.thoughtsTokenCount) || 0);
  }
  usage.days[key] = day;
  saveUsage(usage);
  return day;
...
export function getTodayGeminiUsage(now = new Date()) {
  const usage = loadUsage();
  return normalizeDay(usage.days[todayKey(now)]);
}
