 ...
 * is the requested SLOT (which pacing/retry policy to use), not necessarily
 * which key actually gets billed: currentKey('paid') silently falls back to
 * the free key when no paid key is configured (lib/keyStore.js), so a
 * 'paid'-tier request with no paid key set really spends free-tier quota.
 * recordGeminiUsage() must be told the REAL key, not the requested slot, or
 * byTier.paid (and the "오늘 유료 호출" badge reading it) lights up even
 * when nothing paid ever happened — which is exactly what was observed:
 * paidKeyConfigured:false but the badge still said "유료 호출 9회".
 */
function actualTierFor(tier) {
...
async function runWithRetry(fn, { retries, label, onNotice, tier }) {
  ...
  for (;;) {
    ...
    try {
      ...
      recordGeminiUsage({ label, success: true, tier: actualTierFor(tier), usageMetadata: result?.usageMetadata });
      return result;
    } catch (error) {
      recordGeminiUsage({ label, success: false, tier: actualTierFor(tier) }); // a failed attempt still spent quota (and still billed input tokens on the paid tier, if it really was the paid key)
      lastError = error;
