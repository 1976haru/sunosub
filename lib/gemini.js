...
/*
 * TASK 후속(재조사) — 실측(AI Studio 대시보드, Tier 1)으로 확인됨: RPM
 * 2/1,000, RPD 11/10,000 — 한도의 1%도 안 쓴 상태에서도 오늘 48회 중
 * 35회가 실패했다. 원인은 진짜 429가 아니라 이 함수의 오분류였을
 * 가능성이 높다: status가 429가 아닌 다른 실제 값(예: 400)이어도, 메시지
 * 문자열에 "quota"/"rate limit" 같은 흔한 단어가 어디든 있으면 429로
 * 잘못 판정해 15~120초 백오프를 반복하다 실패했다 — "오래 걸리다 실패"
 * 라는 증상과 정확히 일치한다.
 *
 * @google/genai SDK가 실제 HTTP 상태를 error.status에 채운다는 건 이미
 * 확인된 사실(아래 extractRetryDelayMs 주석 참고). 그래서 실제 상태
 * 코드가 있으면 그것만 믿고, 메시지 문자열은 상태 코드 자체가 없을
 * 때(네트워크 오류 등, status가 0/미정)만 보조로 쓴다. 그 보조 판단에서도
 * "quota"/"rate limit" 같은 일반 단어는 뺐다 — 429/RESOURCE_EXHAUSTED처럼
 * 이 상황에서만 나타나는 구체적인 신호만 남겼다.
 *
 * exported so routes/yt.js's extractWithGemini() can tell a quota/server
 * error apart from a genuine tool-compatibility error without duplicating
 * this detection logic (unlike stripLeadingNumber(), this isn't split
 * across a static page and the server — both sides are this same Node
 * process, so sharing one implementation is the straightforward choice).
 */
export function isRateLimitError(error) {
  const status = Number(error?.status || error?.code || 0);
  if (status) return status === 429;
  const message = String(error?.message || '');
  return /429|RESOURCE_EXHAUSTED/i.test(message);
}
